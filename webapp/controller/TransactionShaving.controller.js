      ...
      onSaveTransaction: function () {
        ...
        if (sOffice && sLaborName) {
          ...
          if (!temp.Date || !temp.Client || !temp.Quantity || !temp.Rate) {
            ...
            // A client the master has never seen is confirmed before anything
            // is written, so a typo never reaches the transaction either.
            this.confirmNewClient(temp.Client, function (bIsNewClient) {
              that._saveShavingTransaction(temp, sLaborName, sOffice, bIsNewClient);
            });
          }
        } else {
          alert("Select Office and Labour");
        }
      },

      _saveShavingTransaction: function (temp, sLaborName, sOffice, bIsNewClient) {
        var that = this;
        var dataLabor = {
